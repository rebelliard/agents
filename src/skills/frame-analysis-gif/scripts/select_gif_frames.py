"""Select representative changed frames from an animated image."""
...
DEFAULT_LOOP_DEDUPE_THRESHOLD = 0.02


def select_gif_frames(
    ...
    loop_dedupe_threshold = (
        options["loopDedupeThreshold"]
        if _is_finite_number(options.get("loopDedupeThreshold"))
        and options["loopDedupeThreshold"] >= 0
        else DEFAULT_LOOP_DEDUPE_THRESHOLD
    )

    first_frame = normalized_frames[0]
    last_distinct_frame = _resolve_last_distinct_frame(
        normalized_frames, loop_dedupe_threshold
    )
    last_distinct_position = normalized_frames.index(last_distinct_frame)

    changed_frames = []
    for position, frame in enumerate(normalized_frames):
        if frame["index"] in {first_frame["index"], last_distinct_frame["index"]}:
            continue
        if position > last_distinct_position:
    ...
    _add_frame(selected_by_index, last_distinct_frame)
    ...
    if len(selected_by_index) < min_frames:
        excluded_indexes = {
            trailing_frame["index"]
            for trailing_frame in normalized_frames[last_distinct_position + 1 :]
        }
...
def normalize_frames(frames: Any) -> list[dict[str, Any]]:
    ...
    for position, frame in enumerate(frames):
        ...
        if _is_finite_number(frame.get("loopDeltaFromFirst")):
            normalized_frame["loopDeltaFromFirst"] = frame["loopDeltaFromFirst"]
        if frame.get("loopDuplicate") is True:
            normalized_frame["loopDuplicate"] = True
...
def _resolve_last_distinct_frame(
    frames: list[dict[str, Any]], loop_dedupe_threshold: float
) -> dict[str, Any]:
    for position in range(len(frames) - 1, 0, -1):
        frame = frames[position]
        repeats_first = frame.get("loopDuplicate") is True or (
            _is_finite_number(frame.get("loopDeltaFromFirst"))
            and frame["loopDeltaFromFirst"] <= loop_dedupe_threshold
        )
        if not repeats_first:
            return frame
    return frames[0]
